...
static void savecmap(const char * filename)
{
    cairo_t *cr;
    cairo_surface_t *surface;

    surface =
        (cairo_surface_t *) cairo_svg_surface_create(filename, 525.0, 525.0);
...
int main(int argc, char ** argv)
{
    ...
    const char * cmapfile = "cmm_cmap.svg";
    printf("Rendering the color map to %s\n", cmapfile);
    savecmap(cmapfile);
